...
namespace OrderService.Api.IntegrationEvents.EventHandlers
{
    public class OrderCreatedIntegrationEventHandler : IIntegrationGenericEventHandler<OrderCreatedIntegrationEvent>
    {
        ...
        {
            _logger.LogInformation("Handling integration event : {IntegrationEventId} at {AppName} - ({@IntegrationEvent})",@event.Id,
                typeof(Program).Namespace,@event);
            ...
            var createOrderCommand = new CreateOrderCommand(
                ...
                @event.CardExpiration,@event.CardSecurityNumber,
                @event.CardTypeId
                );
